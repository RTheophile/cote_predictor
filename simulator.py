...
class Simulator():
    ...
    def simulate_season(self, df_season):
        ...
        df_wins_by_team = pd.merge(home_wins, away_wins, how='inner', left_on=['home_id'], right_on=['away_id']).assign(wins=lambda x: x.wins_home + x.wins_away)[['home_id', 'wins']].rename(
    ...
    def best_of_7(self, proba_win_home, proba_win_away):
        ...
        # Binomial random law
        proba_win_bestof7 = pow(P1, 4) + 4 * pow(P1, 3) * (1 - P1) * (1 - pow(1 - P2, 3)) + 6 * pow(P1, 2) * pow(1 - P1, 2) * (3 * pow(P2, 2) * (1 - P2) + pow(P2, 3)) + 4 * P1 * pow(1 - P1, 3) * pow(
            P2, 3)
